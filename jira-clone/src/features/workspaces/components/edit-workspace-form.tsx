...
export default function EditWorkspaceForm({onCancel, initialValues}:{onCancel?: () => void, initialValues: Workspace}) {
    ...
    const form = useForm<z.infer<typeof editWorkspaceSchema>>({
		...
		const handleDelete = async () => {
			...
			deleteWorkspaceMutation.mutate(initialValues.id);
		}
        ...
        const handleReset = async () => {
            ...
            resetInviteCodeMutation.mutate(initialValues.id, {onSettled: () => {
                router.refresh();
            }});
        }
    ...
    const editWorkspaceMutation = useEditWorkspaceMutation();
        ...
        const resetInviteCodeMutation = useResetInviteCodeMutation();
        ...
        <CardContent className='p-7'>
            <Form {...form}>                
                <form className='space-y-4' onSubmit={form.handleSubmit((data) => {
                    ...
                    editWorkspaceMutation.mutate({data: formData, workspaceId: initialValues.id}, {
                        onSuccess: (workspace) => {
                            form.reset();
